...
install_claude_code() {
    if command -v claude &>/dev/null; then
        ...
        if [ -n "$npm_global_root" ] && [ ! -w "$npm_global_root" ]; then
            log_info "System npm global directory is not writable. Switching to user-local npm prefix..."
            ensure_dir_exists "$NPM_USER_PREFIX"
            npm config set prefix "$NPM_USER_PREFIX"

            local npm_bin_path="$NPM_USER_PREFIX/bin"
            ...
            if [ -f "$HOME/.bashrc" ] && ! grep -q "npm-global/bin" "$HOME/.bashrc"; then
                ...
                log_info "Added npm global bin path to ~/.bashrc"
            fi
        fi
