...
describe("updateForChanges: needs_changes → submitted → full pipeline", () => {
  ...
  it("needs_changes does NOT allow direct transition to pending_review", () => {
    expect(isValidTransition("needs_changes", "pending_review")).toBe(false);
  });
});
...
describe("ALLOWED_TRANSITIONS: completeness for all statuses", () => {
  ...
  it("terminal statuses (approved, rejected) have no outgoing transitions", () => {
    expect(ALLOWED_TRANSITIONS["approved"]).toBeUndefined();
    expect(ALLOWED_TRANSITIONS["rejected"]).toBeUndefined();
  });

  it("generated_locked only transitions to pending_review", () => {
