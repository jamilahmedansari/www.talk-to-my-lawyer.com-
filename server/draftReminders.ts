...
export interface ReminderResult {
  ...
  skipped: number;
  errors: number;
  details: Array<{ letterId: number; status: "sent" | "skipped" | "error"; reason?: string }>;
}
...
export async function processDraftReminders(): Promise<ReminderResult> {
  const result: ReminderResult = { processed: 0, sent: 0, skipped: 0, errors: 0, details: [] };
  ...
  const thresholdDate = new Date(Date.now() - REMINDER_THRESHOLD_HOURS * 60 * 60 * 1000);
  ...
  console.log(`[DraftReminders] Found ${eligibleLetters.length} eligible letters for reminder`);
  ...
  for (const letter of eligibleLetters) {
    try {
      ...
      if (!subscriber?.email) {
        result.skipped++;
        result.details.push({ letterId: letter.id, status: "skipped", reason: "no subscriber email" });
        continue;
      }

      // Calculate how long the draft has been waiting
      const hoursWaiting = (Date.now() - letter.updatedAt.getTime()) / (1000 * 60 * 60);
      ...
      console.log(`[DraftReminders] Reminder sent for letter #${letter.id} to ${subscriber.email} (${Math.round(hoursWaiting)}h waiting)`);
    } catch (err) {
      const msg = err instanceof Error ? err.message : String(err);
      result.errors++;
      result.details.push({ letterId: letter.id, status: "error", reason: msg });
      console.error(`[DraftReminders] Failed to send reminder for letter #${letter.id}:`, msg);
    }
  }

  console.log(`[DraftReminders] Done — sent: ${result.sent}, skipped: ${result.skipped}, errors: ${result.errors}`);
  return result;
}
