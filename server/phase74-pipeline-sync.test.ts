...
describe("Phase 74: Pipeline Status Synchronization", () => {
  ...
  describe("n8n callback — status transitions match direct pipeline", () => {
    it("aligned 3-stage callback transitions: researching → drafting → generated_locked", async () => {
      ...
      const mockReq = {
        ...
        body: {
          ...
          success: true,
          researchPacket: {
            researchSummary: "Comprehensive legal research for California tenant rights...",
            jurisdictionProfile: { country: "US", stateProvince: "CA", city: "LA", authorityHierarchy: ["Federal", "State"] },
            issuesIdentified: ["Unlawful eviction"],
            applicableRules: [{ ruleTitle: "CA Civil Code 1946.2", ruleType: "statute", jurisdiction: "CA", citationText: "§1946.2", sectionOrRule: "1946.2", summary: "Just cause eviction", sourceUrl: "", sourceTitle: "CA Code", relevance: "Primary", confidence: "high" }],
          ...
          draftOutput: {
            draftLetter: "Dear Landlord, This letter serves as formal notice regarding your unlawful eviction attempt...",
            attorneyReviewSummary: "Strong case under CA tenant protection laws.",
            openQuestions: [],
            riskFlags: [],
          },
    ...
    it("audit log fromStatus/toStatus values are accurate for aligned callback", async () => {
      ...
      const mockReq = {
        ...
        body: {
          ...
          researchPacket: {
            ...
            jurisdictionProfile: { country: "US", stateProvince: "TX", city: "", authorityHierarchy: [] },
            issuesIdentified: ["Contract breach"],
            applicableRules: [{ ruleTitle: "UCC", ruleType: "statute", jurisdiction: "TX", citationText: "UCC §2", sectionOrRule: "2", summary: "Sales", sourceUrl: "", sourceTitle: "UCC", relevance: "Primary", confidence: "high" }],
            localJurisdictionElements: [],
            factualDataNeeded: [],
            openQuestions: [],
            riskFlags: [],
          ...
          draftOutput: {
            draftLetter: "Dear Counterparty, We are writing to inform you of a material breach of the agreement dated...",
            attorneyReviewSummary: "Clear breach with documented damages.",
            openQuestions: [],
            riskFlags: [],
          },
          assembledLetter: "Dear Counterparty,\n\nThis firm represents [Client] in connection with the Agreement...\n\nVery truly yours,\nAttorney",
      ...
      const draftingLog = logCalls.find((c: any) => c[0]?.toStatus === "drafting");
      ...
      const lockedLog = logCalls.find((c: any) => c[0]?.toStatus === "generated_locked");
  ...
  describe("Status flow comparison: direct vs n8n", () => {
    it("both pipelines use identical status enum values", () => {
      ...
      const expectedFlow = ["submitted", "researching", "drafting", "generated_locked"];
      ...
      const directFlow = ["submitted", "researching", "drafting", "generated_locked"];
      ...
      const n8nFlow = ["submitted", "researching", "drafting", "generated_locked"];
