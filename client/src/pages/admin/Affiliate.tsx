...
export default function AdminAffiliate() {
  ...
  // ─── Queries ────────────────────────────────────────────────
  const { data: codes, isLoading: codesLoading } = trpc.affiliate.adminAllCodes.useQuery();
  const { data: commissions, isLoading: commissionsLoading } = trpc.affiliate.adminAllCommissions.useQuery();
  const { data: payouts, isLoading: payoutsLoading } = trpc.affiliate.adminAllPayouts.useQuery();
  const { data: performance, isLoading: perfLoading } = trpc.affiliate.adminEmployeePerformance.useQuery();

  // ─── Payout Processing ──────────────────────────────────────
  const [processingPayout, setProcessingPayout] = useState<{ id: number; action: "completed" | "rejected" } | null>(null);
  ...
  const handleProcessPayout = () => {
    ...
    processPayout.mutate({
      ...
      rejectionReason: processingPayout.action === "rejected" ? rejectionReason : undefined,
    });
  };
  ...
  const activeEmployees = performance?.filter(p => p.referralCount > 0).length ?? 0;

  return (
    <AppLayout breadcrumb={[{ label: "Admin", href: "/admin" }, { label: "Affiliate Program" }]}>
      <div className="space-y-6">
        {/* Header */}
        <div className="bg-gradient-to-r from-indigo-700 to-purple-700 rounded-2xl p-6 text-white">
          <h1 className="text-xl font-bold mb-1">Affiliate Program Management</h1>
          <p className="text-indigo-200 text-sm">Manage discount codes, commissions, payouts, and employee performance.</p>
        </div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          <Card>
            <CardContent className="p-4">
              ...
              <p className="text-2xl font-bold">{formatCents(totalCommissions)}</p>
              <p className="text-xs text-muted-foreground mt-0.5">Total Commissions</p>
            </CardContent>
          </Card>
          <Card>
            <CardContent className="p-4">
          ...
          <Card>
            <CardContent className="p-4">
              ...
              <p className="text-xs text-muted-foreground mt-0.5">Discount Codes</p>
            </CardContent>
          </Card>
          <Card>
            <CardContent className="p-4">
              ...
              <p className="text-xs text-muted-foreground mt-0.5">Active Affiliates</p>
            </CardContent>
          </Card>
        </div>
        ...
        <Tabs defaultValue="performance" className="space-y-4">
          <TabsList className="grid grid-cols-2 sm:grid-cols-4 w-full max-w-xl">
            ...
            <TabsTrigger value="payouts"><Wallet className="w-3.5 h-3.5 mr-1" />Payouts</TabsTrigger>
          </TabsList>
          ...
          <TabsContent value="performance">
            <Card>
              <CardHeader>
                ...
                <CardDescription>Overview of each affiliate's referral activity and earnings.</CardDescription>
              </CardHeader>
              <CardContent>
                {perfLoading ? (
                      ...
                      <TableBody>
                        {performance.map((emp) => (
                          <TableRow key={emp.employeeId}>
                            ...
                            <TableCell>
                              {emp.discountCode ? (
                                <Badge variant={emp.codeActive ? "default" : "secondary"} className="font-mono text-xs">
                                  {emp.discountCode}
                                </Badge>
                              ) : (
                            ...
                            <TableCell className="text-center">{emp.usageCount}</TableCell>
                            <TableCell className="text-center">{emp.referralCount}</TableCell>
                            <TableCell className="text-right font-medium">{formatCents(emp.totalEarned)}</TableCell>
                            <TableCell className="text-right text-amber-600">{formatCents(emp.pending)}</TableCell>
                            <TableCell className="text-right text-green-600">{formatCents(emp.paid)}</TableCell>
                          </TableRow>
                        ))}
                      </TableBody>
                    </Table>
          ...
          <TabsContent value="codes">
            <Card>
              <CardHeader>
                ...
                <CardDescription>Manage employee discount codes. Toggle active/inactive status.</CardDescription>
              </CardHeader>
              <CardContent>
                {codesLoading ? (
                  <div className="flex items-center justify-center py-8"><Loader2 className="w-5 h-5 animate-spin" /></div>
                    ...
                    <TableBody>
                      {codes.map((code) => (
                        <TableRow key={code.id}>
                          ...
                          <TableCell className="text-center">{code.discountPercent}%</TableCell>
                          <TableCell className="text-center">{code.usageCount}</TableCell>
                          <TableCell className="text-center">{code.maxUses ?? "∞"}</TableCell>
                          <TableCell className="text-sm">{formatDate(code.createdAt)}</TableCell>
          ...
          <TabsContent value="commissions">
            <Card>
              <CardHeader>
                ...
                <CardDescription>Full ledger of all commissions across employees.</CardDescription>
              </CardHeader>
              <CardContent>
                {commissionsLoading ? (
                  <div className="flex items-center justify-center py-8"><Loader2 className="w-5 h-5 animate-spin" /></div>
          ...
          <TabsContent value="payouts">
            <Card>
              <CardHeader>
                ...
                <CardDescription>Review and process employee payout requests.</CardDescription>
              </CardHeader>
              <CardContent>
                {payoutsLoading ? (
                  <div className="flex items-center justify-center py-8"><Loader2 className="w-5 h-5 animate-spin" /></div>
                            ...
                            {p.status === "pending" ? (
                              <div className="flex items-center justify-end gap-2">
                                <Button
                                  ...
                                  onClick={() => setProcessingPayout({ id: p.id, action: "completed" })}
                                >
                                ...
                                <Button
                                  ...
                                  onClick={() => setProcessingPayout({ id: p.id, action: "rejected" })}
                                >
        ...
        <Dialog open={!!processingPayout} onOpenChange={(open) => { if (!open) { setProcessingPayout(null); setRejectionReason(""); } }}>
          <DialogContent>
            <DialogHeader>
              <DialogTitle>
                {processingPayout?.action === "completed" ? "Approve Payout" : "Reject Payout"}
              </DialogTitle>
              <DialogDescription>
            ...
            <DialogFooter>
              <Button variant="outline" onClick={() => { setProcessingPayout(null); setRejectionReason(""); }}>
                Cancel
              </Button>
              <Button
                onClick={handleProcessPayout}
                disabled={processPayout.isPending || (processingPayout?.action === "rejected" && !rejectionReason.trim())}
                className={processingPayout?.action === "completed" ? "bg-green-600 hover:bg-green-700" : "bg-red-600 hover:bg-red-700"}
              >
                {processPayout.isPending && <Loader2 className="w-4 h-4 mr-1 animate-spin" />}
                {processingPayout?.action === "completed" ? "Confirm Approval" : "Confirm Rejection"}
              </Button>
            </DialogFooter>
          </DialogContent>
        </Dialog>
